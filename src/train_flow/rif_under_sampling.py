...
# st = time.time()
# x_oh_rif_tl, y_oh_rif_tl = prepro_modules.data_resampling(x_oh_train, y_oh_train, sampling_method='RUS')
...
st2 = time.time()
x_noh_rif_tl, y_noh_rif_tl = prepro_modules.data_resampling(x_noh_train, y_noh_train, sampling_method='RUS')
x_noh_rif_tl.to_csv(os.path.join(no_onehot_path, 'x_train_rif_rus.csv'), index=False)
y_noh_rif_tl.to_csv(os.path.join(no_onehot_path, 'y_train_rif_rus.csv'), index=False)
print(time.time() - st2)
