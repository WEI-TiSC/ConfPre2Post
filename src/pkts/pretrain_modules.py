...
def objective(feats, labels, param_grid, trial, metric='f1-macro', use_model='LGBM', k_fold=6):
    ...
    for idx, (train_idx, cv_idx) in enumerate(stratified_k_fold.split(feats, labels)):
        ...
        if use_model == 'LGBM':
            pre_train_LGBM(x_train, x_cv, y_train, y_cv, param_grid, cv_scores, idx, metric)
        elif use_model == 'RF':
            pre_train_RF(x_train, x_cv, y_train, y_cv, param_grid, cv_scores, idx, metric)
        elif use_model == 'CB':
            pre_train_CB(x_train, x_cv, y_train, y_cv, param_grid, cv_scores, idx, metric)
        else:
...
def param_search_by_optuna(feats, labels, study_name='LGBM', metric='f1-macro', n_trials=300, one_hot=False):
    ...
    study_info = study_name + '_Pretrain'
    ...
    result_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
                              ...
                              f'{datetime.date.today()}_{one_hot_str}_{metric}')
